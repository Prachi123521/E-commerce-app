...
const items = [
  ...
  { src: image16.src, title: "Women's Boot", category: "women" },
  { src: image17.src, title: "Men's Best Sellers", category: "men" },
  { src: image17.src, title: "Men's Sweaters", category: "men" },
  { src: image17.src, title: "Men's Trousers", category: "men" },
  { src: image17.src, title: "Men's Trouser", category: "men" },
];
...
export default function Search() {
  ...
  const filteredItems = items.filter((item) => {
    ...
    const matchesSearch =
      matchesTitle ||
      `${item.category} ${item.title}`.toLowerCase().includes(search);
    return matchesSearch && matchesCategory;
  });

  const shouldShowPopular = searchTerm === "" && selectedCategory === "all";
    ...
    <div className="px-4 md:px-10 lg:px-20">
      ...
      {!shouldShowPopular && (
        <div className="mt-6 lg:px-20">
          {searchTerm && (
            <p className="tracking-wide font-normal text-sm mb-4">
              Search Results
            </p>
          )}
      ...
      {shouldShowPopular && (
        <>
          ...
          <div className="mt-4 grid grid-cols-1  md:grid-cols-4 lg:px-20 gap-x-4 mb-12">
            {items.map((item) => (
              <div key={item.title} className="flex flex-col">
                <img src={item.src} alt={item.title} className="w-full" />
                <p className="mt-2 font-normal text-[14px] font-maison-neue underline hover:decoration-blue-400 cursor-pointer">
